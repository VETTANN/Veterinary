...
public class MainApp {
    ...
    // ── Змінено: factory тепер поле екземпляра (не static) ──────────────────
    // Це дозволяє тестам підставити власну SessionFactory через setFactory(),
    // не запускаючи main() і не потребуючи реального PostgreSQL.
    ...
    // ── main() — лише точка входу, ініціалізує factory і делегує до run() ───
    ...
    // ── run() — логіка меню, використовує this.factory ──────────────────────
    public void run() {
    ...
    // ── printMenu() — окремий метод, щоб JaCoCo бачив його покритим ─────────
    void printMenu() {
        System.out.println("\n-------------------------------------------");
    ...
    // ── loadTestData() — тепер instance-метод ────────────────────────────────
    void loadTestData() {
        try (Session session = factory.openSession()) {
            ...
            session.persist(new Appointment(p1, v1, LocalDate.parse("2024-09-14"), LocalTime.parse("10:00:00"), "Комплексне щеплення, огляд", true));

            Owner o2 = new Owner("Сидоренко", "Марія", "Олексіївна", "м. Київ", "Лесі Українки", "5", null, "380502222222");
            Pet p2 = new Pet(o2, "Рекс", "собака");
            session.persist(o2); session.persist(p2);
            session.persist(new Appointment(p2, v1, LocalDate.parse("2024-09-14"), LocalTime.parse("11:30:00"), "Щеплення від сказу", true));

            Owner o3 = new Owner("Бондаренко", "Дмитро", "Сергійович", "м. Київ", "просп. Перемоги", "33", null, "380503333333");
            Pet p3 = new Pet(o3, "Муся", "кішка");
            session.persist(o3); session.persist(p3);
            session.persist(new Appointment(p3, v2, LocalDate.parse("2024-09-14"), LocalTime.parse("14:20:00"), "Огляд після операції", false));
            ...
            session.persist(new Appointment(p4, v3, LocalDate.parse("2024-09-14"), LocalTime.parse("15:10:00"), "Чищення зубів", false));

            Owner o5 = new Owner("Коваленко", "Сергій", "Володимирович", "м. Київ", "Саксаганського", "7", null, "380505555555");
            Pet p5 = new Pet(o5, "Зоря", "кішка");
            session.persist(o5); session.persist(p5);
            session.persist(new Appointment(p5, v3, LocalDate.parse("2024-09-15"), LocalTime.parse("09:15:00"), "Лікування захворювання шкіри", false));

            Owner o6 = new Owner("Марченко", "Ірина", "Олегівна", "м. Київ", "Богдана Хмельницького", "10", null, "380506666666");
            Pet p6 = new Pet(o6, "Буч", "собака");
            session.persist(o6); session.persist(p6);
            session.persist(new Appointment(p6, v3, LocalDate.parse("2024-09-15"), LocalTime.parse("11:00:00"), "Комплексне щеплення, огляд", true));

            Owner o7 = new Owner("Шевчук", "Павло", "Миколайович", "м. Київ", "Антоновича", "72", null, "380507777777");
            Pet p7 = new Pet(o7, "Сімба", "кіт");
            session.persist(o7); session.persist(p7);
            session.persist(new Appointment(p7, v1, LocalDate.parse("2024-09-15"), LocalTime.parse("14:45:00"), "Кастрація", false));

            Owner o8 = new Owner("Гончарук", "Наталія", "Вікторівна", "м. Київ", "Велика Васильківська", "7", null, "380508888888");
            Pet p8 = new Pet(o8, "Лапа", "собака");
            session.persist(o8); session.persist(p8);
            session.persist(new Appointment(p8, v2, LocalDate.parse("2024-09-16"), LocalTime.parse("10:30:00"), "Щеплення від сказу", true));

            Owner o9 = new Owner("Лисенко", "Артем", "Олексійович", "м. Київ", "Дегтярівська", "33", null, "380509999999");
            Pet p9 = new Pet(o9, "Мурка", "кішка");
            session.persist(o9); session.persist(p9);
            session.persist(new Appointment(p9, v3, LocalDate.parse("2024-09-16"), LocalTime.parse("12:15:00"), "Лікування кон'юнктивіту", false));

            Owner o10 = new Owner("Павленко", "Катерина", "Сергіївна", "м. Київ", "Олени Теліги", "18", null, "380501010101");
            Pet p10 = new Pet(o10, "Тедді", "собака");
            session.persist(o10); session.persist(p10);
            session.persist(new Appointment(p10, v1, LocalDate.parse("2024-09-16"), LocalTime.parse("15:00:00"), "Комплексне щеплення, огляд", true));

            Owner o11 = new Owner("Ковальчук", "Михайло", "Іванович", "м. Київ", "Дорогожицька", "2", null, "380501112131");
            Pet p11 = new Pet(o11, "Річ", "кіт");
            session.persist(o11); session.persist(p11);
            session.persist(new Appointment(p11, v2, LocalDate.parse("2024-09-17"), LocalTime.parse("09:45:00"), "Огляд після лікування", false));

            session.persist(new Appointment(p3, v3, LocalDate.parse("2024-09-17"), LocalTime.parse("13:30:00"), "Комплексне щеплення, огляд", true));
    ...
    // ── showReport() — тепер instance-метод ──────────────────────────────────
    void showReport() {
        try (Session session = factory.openSession()) {
    ...
    // ── clearDatabase() — тепер instance-метод ───────────────────────────────
    void clearDatabase() {
        try (Session session = factory.openSession()) {
            Transaction tx = session.beginTransaction();
